...
test('Server starts and serves HTML containing "List of frameworks"', async (t) => {
  ...
  const frakPath = path.resolve(__dirname, '../packages/bin/frak.js');

  await new Promise((resolve, reject) => {
    // Spawn the process running "frak dev" from the tests/sample directory
    server = spawn('node', [frakPath, 'dev'], {
