...
import { createGrahpqlServer } from './graphql.js';
import Router from './router.js';
import { cwd } from 'node:process';
const debug = makeDebug('frak:server');
...
class Server {
  ...
  async create() {
    ...
    console.log(
      `Frak Server (PID: ${process.pid}) started on port: ${this.port}`
    );
    // Load routes
    ...
    // Add GraphQL server
    if (this.gql) {
      await createGrahpqlServer(app, httpServer, result.schemas);
    }
    ...
    const localFrameworkPaths = {
      '/_framework/frak.js': ['..', 'frontend', 'frak.js'],
    ...
    if (process.env.NODE_ENV === 'production') {
      app.use('/components', express.static('.frak/components'));
    } else {
